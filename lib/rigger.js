...
function Rigger(opts) {
    ...
    this.targetType = this.opts.targetType || this.filetype;
    ...
    async.map(
        ...
        function(err, result) {
            ...
            if (! err) {
                ...
                if (result.length > 0) {
                    var conversion = rigger._getConversion(rigger.filetype);
                    
                    rigger.convert(conversion, result.join('\n'), function(err, content) {
                        if (err) {
...
Rigger.prototype._expandIncludes = function(settings, line, callback) {
    ...
    this[action].call(this, match, settings, function(err, content, conversion) {
        ...
        rigger.convert(conversion, content, function(convertErr, convertedContent) {
            ...
            // parse the lines
            async.map(
                (convertedContent || '').split(reLineBreak),
...
Rigger.prototype._getConversion = function(ext) {
    ext = (ext || '').replace(reLeadingDot, '').toLowerCase();
