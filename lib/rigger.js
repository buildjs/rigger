...
Rigger.prototype._getSingle = function(target, callback) {
    ...
    // otherwise, we'll do a little more work
        ...
        testTargets.forEach(function(target, index) {
            ...
            if (path.extname(target) === '') {
                // first look for the non-minified version
                testTargets.push(target + '.' + rigger.filetype);

                // next look for the minified version
                testTargets.push(target + '.min.' + rigger.filetype);
            }
        });
