...
Rigger.prototype._expandIncludes = function(line, callback) {
    ...
    for (ii = regexes.length; ii--; ) {
        ...
        if (regexes[ii].test(line)) {
            ...
            if (targetExt && targetExt.slice(1).toLowerCase() !== rigger.filetype.toLowerCase()) {
    ...
    if (! getTarget) return callback(null, line);

    // if we have cached results, then return those
    if (rigger.memcache[getTarget]) return callback(null, rigger.memcache[getTarget]);
    ...
    rigger.get(getTarget, function(err, content) {
        ...
        async.map(
            ...
            rigger._recombinate.bind(rigger, getTarget, conversion, callback)
        );
    });
};
