...
Rigger.prototype.include = function(match, callback) {
...
Rigger.prototype.plugin = function(match, callback) {
    ...
    try {
        // FIXME: hacky
        while (packagePath && packagePath != lastPackagePath && (! path.existsSync(path.join(packagePath, 'package.json')))) {
...
Rigger.prototype.resolve = function(targetPath) {
    var scopeRelative = path.resolve(rigger.csd, targetPath),
        workingRelative = path.resolve(rigger.cwd, targetPath);
        
    return path.existsSync(scopeRelative) ? scopeRelative : workingRelative;
};
...
Rigger.prototype._expandIncludes = function(line, callback) {
    ...
    this[action].call(this, match, function(err, content, conversion) {
        ...
        async.map(
            ...
            function(line, itemCallback) {
                rigger._expandIncludes(match[1] + line, itemCallback);
            },
            rigger._recombinate.bind(rigger, conversion, callback)
        );
    });
};
...
Rigger.prototype._recombinate = function(conversion, callback, processingError, results) {
