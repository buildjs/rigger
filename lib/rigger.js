...
Rigger.prototype.include = function(match, callback) {
    var target = match[2].replace(reTrailingDot, '').replace(reQuotesLeadAndTrail, ''),
...
Rigger.prototype.plugin = function(match, callback) {
    ...
    if (typeof plugin == 'function') {
        plugin.apply(scope, [this].concat(match.slice(3)));
    }
    else {
