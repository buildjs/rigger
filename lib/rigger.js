...
function Rigger(opts) {
    ...
    this.baseSettings = opts.settings || {};
...
Rigger.prototype.convert = function(conversion, input, callback) {
    // get the converter
    var steps = [].concat(this.converters[conversion] || []);
...
Rigger.prototype.write = function(data, all) {
    var rigger = this, lines,
        settings = _.extend({}, this.baseSettings);
...
Rigger.prototype.include = function(match, settings, callback) {
    ...
    debug('include: ' + target + ' requested, file ext = ' + targetExt + ', context: ' + this.context);
    ...
    // otherwise, we'll do a little more work
        ...
        async.detect(testTargets, _exists, function(realTarget) {
            if (! realTarget) {
                ...
                return callback(rigger.tolerant ? null : new Error('Unable to find target for: ' + target));
            }
